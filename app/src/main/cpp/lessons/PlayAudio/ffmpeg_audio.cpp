...
int createFFmpegEngine(const char *url) {
    ...
    if (!global_dAudio.pACodec) {
        LOG_E("do not find matched codec for %s", global_dAudio.pFmtCtx->audio_codec->name);
        releaseFFmpegEngine();
        return -1;
    }
...
//    av_dict_set(&opts, "b", "2.5M", 0);
    ...
    if (ret < 0) {
        LOG_E("open %s codec failed.", global_dAudio.pFmtCtx->audio_codec->name);
        releaseFFmpegEngine();
        return -1;
    }
...
int decodeAudio(uint8_t **data) {
    ...
    while (avcodec_receive_frame(global_dAudio.pACodecCtx, global_dAudio.pAFrame) == 0) {
        LOG_D("读取一帧音频数据,frameSize=%d", global_dAudio.pAFrame->nb_samples);
        break;
    }
    // 重采样
